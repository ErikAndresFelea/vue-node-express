...
<div>
  <div>
    <h3>Versiones</h3>
    <VersionTableComp @reset-version="resetVersion" @show-version="showVersion" :versions="versions" />
  </div>
  <div>
  ...
  methods: {
    ...
    async resetVersion(resetVersion) {
      if(confirm('¿Seguro quieres RESTAURAR esta transcripcion?')) {
        const res = await fetch(`api/transcriptions/${resetVersion.id}/${resetVersion.version}`, {
          method: 'DELETE'
        })
        ...
        const resOld = await fetch(`api/transcriptions/${resetVersion.id}/${resetVersion.version}`, {
          method: 'PUT',
          headers: {
        ...
        resOld.status === 200 ? alert('Transcripcione restaurada') : alert('ERROR al restaurar la transcripcion')
      }
    }
  },

  emits: [
      'show-get-u', 'upd-transc', 'reset-version', 'show-version'
    ]
}
</script>
